...
from requests.utils import urlparse, DEFAULT_PORTS
...
class ODOO(object):
    ...
    *Python 3:*
    ...
    def __init__(self, url='http://localhost:8069', 
        ...
        try:
            self._client = Client(self._root_url, timeout=timeout, version=version)
    ...
    def json(self, url, params):
        ...
        *Python 3:*
        ...
        :raise: `urllib.error.URLError` (connection error)
        """
        self._client.url = ''.join([self._root_url,url]) 
        data = self._client.call(**params)
        if data.get('error'):
    ...
    def http(self, url, data=None, headers=None):
        ...
        *Python 3:*
        ...
        :raise: `urllib.error.URLError` (connection error)
        """
        return self._client.request.get(''.join([self._root_url,url]) , data=data, headers=headers)
        ...
        if data.get('type') != cls.__name__:
            raise error.InternalError(
                "'{0}' session is not of type '{1}'".format(
                    name, cls.__name__))
        odoo = cls("{protocol}://{host}:{port}".format(
            host=data['host'],
